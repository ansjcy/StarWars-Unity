...
public class MultiplayerController : MonoBehaviour {
	...
	void RegisterEvents()
	{
		view.MyPlayerIsReady += (object sender, System.EventArgs e) => GetComponent<NetworkView>().RPC ("IsReady", RPCMode.Others, view.MyName);
	...
	void HandleHitObject (object sender, GlobalGameState.HitObjectArgs e)
	{
		...
		if (e.hitRaycast.transform.GetComponent<PlayerShield>() != null)
		{
			...
			scoreBoard.AddToPlayerScoreByGuid( e.weaponObject.GetComponent<NetworkView>().owner.guid, scoreToAdd );
			GetComponent<NetworkView>().RPC ("SetPlayerScores", RPCMode.Others, scoreBoard.Serialize());

			if (scoreBoard.GetPlayerScoreByGui(e.weaponObject.GetComponent<NetworkView>().owner.guid) >= TargetScoreToWin)
			{
				// We have a winner.
				winner = scoreBoard.GetPlayerNameByGuid(e.weaponObject.GetComponent<NetworkView>().owner.guid);
	...
	void GameOver()
	{
		GetComponent<NetworkView>().RPC ("ServerCalledGameOver", RPCMode.All, winner);
		OnGameOver();
	}
	...
	void OnConnectedToServer()
	{
		...
		GetComponent<NetworkView>().RPC("JoinPlayer", RPCMode.Others, view.MyName);
	...
	void StartGame()
	{
		...
		scoreBoard.AddPlayer(view.MyName,player.GetComponent<NetworkView>().owner.guid);

		string serializedData = scoreBoard.Serialize();
		//Debug.Log ("Sending data : " + serializedData);
		GetComponent<NetworkView>().RPC ("SetPlayerScores", RPCMode.Others, serializedData);
	}
	...
	GameObject SpawnPlayer()
	{
		...
		//CameraObjectToAttachPlayer.GetComponent<Thruster> ().addForceAtPosition = player.transform;
		if (Radar != null)
		{
	...
	void JoinPlayer(string playerName)
	{
		...
		GetComponent<NetworkView>().RPC ("SetPlayerData", RPCMode.Others, view.Serialize());
	}
