...
//			Debug.Log ("yes");
	...
	void Update () {
		...
		// Is this me?
		if (!GetComponent<NetworkView>().isMine && !is_menu_ship)
		{
			SyncedMovement();
			return;
		}
		...
		//Debug.Log ("start detecting this");
...
//		}
		...
		if (ButtonFire.fire && Time.time > nextFire) {
	...
	void SyncedMovement()
	{
		...
		GetComponent<Rigidbody>().rotation = syncCurrentRotation;
		GetComponent<Rigidbody>().angularVelocity = syncCurrentAngVelocity;
	}
	...
	void FixedUpdate () {
		...
		float horizontal = Input.gyro.rotationRate.y;//Input.GetAxis(horizontal_string);
		float vertical = Input.gyro.rotationRate.z;//Input.GetAxis (vertical_string);
	...
	void OnSerializeNetworkView(BitStream stream, NetworkMessageInfo info)
	{
		...
		if (stream.isWriting)
		{
			...
			angVelocity = GetComponent<Rigidbody>().angularVelocity;
